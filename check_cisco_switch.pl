...
# Configuration
my $S_conf_changed = "1.3.6.1.4.1.9.9.43.1.1.1.0";
my $S_run_changed = "1.3.6.1.4.1.9.9.43.1.1.2.0";
my $S_conf_saved = "1.3.6.1.4.1.9.9.43.1.1.3.0";
my $S_up_time = "1.3.6.1.2.1.1.3.0";
...
sub check_oid_return {
	...
	if ( $oid_value == '' ) {
		print "$parameter_name: OK - Remote device does not return data for $parameter_name\n";
		exit(0);
	}
}
...
# Validate Warning
if("$check_type" ne "temp" and "$check_type" ne "conf") {
...
### CPU Load ###
	...
	$perf = "cpu_5s=$load_5s;$warn;$crit cpu_1m=$load_1m cpu_5m=$load_5m";
...
### Changed and not saved configuration ###
} elsif($check_type eq "conf"){
	$snmp_session->translate(['-timeticks' => 0]);
	my $lastChange = $snmp_session->get_request(-varbindlist => [ $S_conf_changed ]);
	my $runChange = $snmp_session->get_request(-varbindlist => [ $S_run_changed ]);
	...
	if ($lastChange->{$S_conf_changed} == "" or $lastSave->{$S_conf_saved} == ""){
		...
		$stat = 2;
		$msg = "Configuration: CRIT - Configuration not saved";
	}else{
		$stat = 0;
		$msg = "Configuration: OK - Configuration saved";
	}
print ("Uptime:\t$uptime->{$S_up_time}\n");
print ("Last change:\t$lastChange->{$S_conf_changed}\n");
print ("Last run change:\t$runChange->{$S_run_changed}\n");
print ("Last save:\t$lastSave->{$S_conf_saved}\n");
} else {
	FSyntaxError();
}
